...
public class KorisnikPregled extends JFrame {
	...
	public void PrikazPodataka() {
		...
		String query = "SELECT i.ID, CONCAT_WS('',p.Naziv) as 'Naziv predstave', CONCAT_WS('',s.Ime) as 'Naziv sale', i.Dan FROM izvodjenje_predstave i, predstave p, sala s WHERE i.ID_P = p.ID AND i.ID_S = s.ID ";
